...
public static class PageExtensions
{
    ...
    public static async Task<PageModel<T>> ToPageModel<T>(this IAsyncQueryable<T> source, IDynamicPaging complexModel, bool applyFiltering = true) where T : class
    {
        var page = await GetPageInfo(source, complexModel);
        if (applyFiltering) source = source.ApplyFilter(complexModel);
        source = source.ApplyPaging(complexModel);
    ...
    private static async Task<(int page, int pageSize, int count)> GetPageInfo<T>(IAsyncQueryable<T> source, IDynamicPaging complexModel) where T : class
    {
    ...
    public static async Task<PageModel<T>> ToPageModel<T>(this IAsyncEnumerable<T> source, IDynamicPaging complexModel, bool applyFiltering = true) where T : class =>
